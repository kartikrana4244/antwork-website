        ...
        <div className="grid gap-12 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="pt-1">
              ...
              <form className="mt-3 flex min-h-[44px] flex-col gap-2 sm:flex-row" onSubmit={(e) => e.preventDefault()} aria-label="Newsletter signup">
                <input
                  ...
                  className="min-h-[44px] flex-1 rounded-md border border-white/10 bg-white/[0.04] px-4 py-2.5 text-sm text-white placeholder-white/25 transition-colors focus:border-[#F2C94C] focus:outline-none focus:ring-1 focus:ring-[#F2C94C]/30"
                  aria-label="Email for newsletter"
                />
                <button type="submit" className="min-h-[44px] shrink-0 rounded-md bg-[#F2C94C] px-5 py-2.5 text-sm font-semibold text-[#1A1A1A] transition-all duration-200 hover:bg-[#E0B429]">
                  Subscribe
                </button>
              </form>
            </div>
          ...
          <div className="min-w-0">
            ...
            <ul className="mt-5 space-y-2.5">
              {quickLinks.map((link) => (
                <li key={link.href}>
                  <Link href={link.href} className="text-sm text-white/40 transition-colors hover:text-[#F2C94C]">{link.label}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="min-w-0 space-y-7">
            <div>
              ...
              <address className="mt-5 space-y-2 text-sm not-italic text-white/40">
                ...
                <div className="space-y-2 pt-3">
                  ...
                  <p><a href="mailto:office@antworkconsultants.com" className="break-words transition-colors hover:text-[#F2C94C]">office@antworkconsultants.com</a></p>
                  <p className="pt-1 text-xs font-semibold uppercase tracking-[0.15em] text-white/20">Alternate Email</p>
                  <p><a href="mailto:antworkconsultants@gmail.com" className="break-words transition-colors hover:text-[#F2C94C]">antworkconsultants@gmail.com</a></p>
                  <p className="pt-1 text-xs font-semibold uppercase tracking-[0.15em] text-white/20">Phone</p>
                  <p><a href="tel:+919217167326" className="transition-colors hover:text-[#F2C94C]">+91 9217167326</a></p>
                </div>
              </address>
            </div>
            ...
            <div>
              ...
              <div className="mt-4 flex gap-4">
                <a href="https://www.linkedin.com/company/antwork-consultants/posts/?feedView=all" target="_blank" rel="noopener noreferrer" className="flex h-10 w-10 items-center justify-center rounded-full border border-white/10 text-white/40 transition-all duration-300 hover:border-[#F2C94C] hover:text-[#F2C94C]" aria-label="LinkedIn">
                  <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                  </svg>
                </a>
                <a href="https://www.instagram.com/antworkconsultants/?hl=en" target="_blank" rel="noopener noreferrer" className="flex h-10 w-10 items-center justify-center rounded-full border border-white/10 text-white/40 transition-all duration-300 hover:border-[#F2C94C] hover:text-[#F2C94C]" aria-label="Instagram">
                  <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        ...
        <div className="mt-14 border-t border-white/8 pt-7">
          <div className="flex flex-col items-center gap-4 text-center text-xs text-white/25 sm:flex-row sm:justify-between sm:text-sm">
            ...
            <div className="flex flex-wrap justify-center gap-5 sm:gap-6">
              <Link href="/privacy" className="transition-colors hover:text-[#F2C94C]">Privacy Policy</Link>
              <Link href="/terms" className="transition-colors hover:text-[#F2C94C]">Terms</Link>
              <Link href="/disclaimer" className="transition-colors hover:text-[#F2C94C]">Disclaimer</Link>
            </div>
          </div>
        </div>
      </div>
